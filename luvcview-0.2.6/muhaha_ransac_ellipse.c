...
u08 solve_ellipse(double* conic_param, double* ellipse_param)
{
...
tV2d* normalize_edge_point	(double* dis_scale, tV2d* nor_center, int ep_num)
{
  double sumx = 0, sumy = 0;
  double sumdis = 0;
  tV2d *edge;
  ...
  *dis_scale = sqrt((double)2)*ep_num/sumdis;
  ...
  tV2d *edge_point_nor = (tV2d*)malloc(sizeof(tV2d)*ep_num);
  for (i = 0; i < ep_num; i++) {
    edge = &gM_edge_point[i];
    edge_point_nor[i].x = (edge->x - nor_center->x) * *dis_scale;
    edge_point_nor[i].y = (edge->y - nor_center->y) * *dis_scale;
...
void denormalize_ellipse_param(double* par, double* normailized_par, double dis_scale, tV2d nor_center)
{
...
int*		simple_fit		(int width, int height, int* return_max_inliers_num)
{
	int i;
